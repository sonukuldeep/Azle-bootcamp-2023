...
import { Move as controls, keyDown, keyUp } from '../lib/Controls';

function Canvas() {
  const gameOverUI = useRef<HTMLDivElement>(null);
  const gameWonUI = useRef<HTMLDivElement>(null);
  const canvsRef = useRef<HTMLCanvasElement>(null);
  const startGame = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    if (!gameOverUI || !gameOverUI.current) return;
    if (!gameWonUI || !gameWonUI.current) return;
    if (!startGame || !startGame.current) return;
    if (!startBtn || !startBtn.current) return;
    canvasFunction(
      canvas,
      ctx,
      gameOverUI.current,
  ...
  return (
    <>
      <h2 style={{ textAlign: 'center' }}>
        Complete the game,
        <br />
        collect the coupon and
        <br />
        exchange it for TGS token in fosset
      </h2>
      <div className="canvas-wrapper">
        ...
        <UIElements
          ...
          startGame={startGame}
          startBtn={startBtn}
          coupon={coupon[Math.floor(Math.random() * coupon.length)]}
...
function canvasFunction(
  ...
  gameOverUI: HTMLElement,
  gameWonUI: HTMLElement,
  startGame: HTMLDivElement,
  startBtn: HTMLButtonElement,
  setShowCoupon: Dispatch<SetStateAction<boolean>>,
) {
  ...
  //custom event
  const PowerUpEvent = new CustomEvent('powerUpEvent', {
    detail: { power: { type: '', number: -1 } },
  });
  ...
  class Ball {
    ...
    update(platform: Platform) {
      ...
      if (detectCollision(circle, rectangle)) {
        ...
        if (ratio <= 0) this.vx = -3;
        else if (ratio < 0.2) this.vx = -2;
        else if (ratio < 0.4) this.vx = -1;
        else if (ratio < 0.6) this.vx = 0;
        else if (ratio < 0.8) this.vx = 1;
        else if (ratio < 1) this.vx = 2;
        else if (ratio >= 1) this.vx = 3;
  ...
  class Effect {
    ...
    constructor(canvas: HTMLCanvasElement) {
      ...
      this.noOfRows = 3;
      this.tileAdjustment =
        (this.width - this.noOfTilesPerRow * Tile.width + Tile.gap) * 0.5;
    ...
    createParticle() {
      for (let i = 2; i <= this.noOfRows + 1; i++) {
        //starting from to give myself 2xheight for charater placement
        for (let j = 0; j < this.noOfTilesPerRow; j++) {
          const spawnCharaterOnFirstRow = i === 2 ? true : false;
          this.tiles.push(
            new Tile(
              Tile.width * j + this.tileAdjustment,
              Tile.height * i,
              spawnCharaterOnFirstRow,
            ),
          );
        }
      }
    ...
    handleParticles(context: CanvasRenderingContext2D) {
      ...
      this.tiles.forEach((tile, index) => {
        ...
        if (tile.shouldDrawNains) {
          ...
          if (nain.nainsIsPresent) {
            const rectangle1 = {
              ...
              y: nain.y,
              width: nain.width,
              height: nain.height - nain.verticalShift,
              id: nain.nainsId,
            }; // zero since height was originally shifted 25 px
            const rectangle2 = {
              x: this.platform.x,
              y: this.platform.y,
        ...
        this.platform.bullet.forEach((bullet) =>
          bullet.deactivateBullet(tile, nain),
        );
      });
  ...
  class Platform {
    ...
    draw(context: CanvasRenderingContext2D) {
      // controls
      if (
        this.shake.shake < this.platformHitPanelty &&
        this.x - 10 * controls.x > 0 &&
        this.x + this.width - 10 * controls.x < this.canvasWidth
      )
      ...
      context.beginPath();
      context.moveTo(
        this.x + this.shake.vibrateX + this.radius,
        this.y + this.shake.vibrateY,
      );
      context.arcTo(
        this.x + this.shake.vibrateX + this.width,
        this.y + this.shake.vibrateY,
        this.x + this.shake.vibrateX + this.width,
        this.y + this.shake.vibrateY + this.height,
        this.radius,
      );
      context.arcTo(
        this.x + this.shake.vibrateX + this.width,
        this.y + this.shake.vibrateY + this.height,
        this.x + this.shake.vibrateX,
        this.y + this.shake.vibrateY + this.height,
        this.radius,
      );
      context.arcTo(
      ...
      gunActive &&
        context.arc(
          this.x + this.shake.vibrateX + this.bulletOffset,
          this.y + this.shake.vibrateY,
          5,
          Math.PI,
          2 * Math.PI,
      ...
      gunActive &&
        context.arc(
          this.x + this.shake.vibrateX + this.width - this.bulletOffset,
          this.y + this.shake.vibrateY,
          5,
          Math.PI,
          2 * Math.PI,
  ...
  class Bullet {
    ...
    deactivateBullet(tile: Tile, nain: Character) {
      if (
        !tile.deactivate &&
        this.x > tile.x &&
        this.x < tile.x + tile.effectiveWidth &&
        this.y < tile.y + tile.effectiveHeight
      ) {
        this.deactive = true;
  ...
  class Tile {
    ...
    constructor(x: number, y: number, spawn = false) {
      ...
      this.shouldDrawNains =
        spawn && Math.floor(Math.random() * 2) === 1 ? true : false;
  ...
  class Pot {
    ...
    reset(platform: Platform) {
      this.angle = Math.atan2(
        platform.y - (this.y + this.verticalShift + 30),
        platform.x + platform.width / 2 - this.x,
      );
      this.potx = this.x;
      this.poty = this.y;
      this.potV = 10;
    ...
    draw(
      ...
      if (status === 'aim') {
        context.drawImage(
          this.pot,
          this.potNumber * this.potWidth,
          this.spriteSheetRowNumber * this.potHeight,
          this.potWidth,
          this.potHeight,
          this.x,
          this.y - this.verticalShift - 20,
          this.potWidth,
          this.potHeight,
        );
        ...
        context.drawImage(
          ...
          this.potHeight,
          this.potx,
          this.poty - this.verticalShift - 20,
          this.potWidth,
          this.potHeight,
        );
        ...
        this.potV *= 1.01;
        //pots
        const rectangle1: IRectangleCollisionProps = {
          x: this.potx,
          y: this.poty,
          width: this.potWidth,
        ...
        const rectangle2: IRectangleCollisionProps = {
          x: platform.x,
          y: platform.y,
          width: platform.width,
          height: platform.height,
        ...
        if (!this.potCollided) {
          ...
          context.drawImage(
            this.explosionImage,
            this.explodePotKeyFrame * this.explositionWidth,
            0,
            this.explositionWidth,
            this.explositionHeight,
            this.collideX,
            this.collideY,
            this.explositionWidth,
            this.explositionHeight,
    ...
    drawNains(context: CanvasRenderingContext2D, platform: Platform) {
      ...
      if (this.nainsIsPresent && !this.shouldNainsFall && !this.showPowerUp) {
        ...
        if (this.nainsFrame >= 0 && this.nainsFrame < 200) {
          context.drawImage(
            this.nainsImage,
            Math.floor(this.nainsIdea) * this.width,
            0,
            this.width,
            this.height,
            this.x,
            this.y - this.verticalShift,
            this.width,
            this.height,
          );
          // this.potStatus = 'idle'
        ...
        this.nainsIsPresent &&
        this.shouldNainsFall &&
        !this.showPowerUp
      ) {
        // show falling nains
        context.drawImage(
          this.nainsImage,
          Math.floor(this.nainsFall) * this.width,
          6 * this.height,
          this.width,
          this.height,
          this.x,
        ...
        context.drawImage(
          this.powerUpImage,
          this.powerUp * this.powerUpWidth,
          0,
          this.powerUpWidth,
          this.powerUpHeight,
          this.x - this.powerUpWidth / 4,
          this.y - this.verticalShift,
          this.powerUpWidth,
          this.powerUpHeight,
        );
  ...
  function callGameOver(won = false) {
    Array.from(document.querySelectorAll('.score .score-count')).forEach(
      (ui) => (ui.innerHTML = score.toString()),
    );
    !won
      ? gameOverUI.classList.add('activate')
      : gameWonUI.classList.add('activate');
    gameOver === false &&
      setTimeout(() => {
    ...
    ({ detail }: IPowerUpEventProps) => {
      ...
      switch (detail?.power.number) {
        ...
        case 4: {
          ...
          minWidth <= 24
            ? (effect.platform.width = 24)
            : (effect.platform.width = minWidth);
          break;
        }
        case 5: {
          //power.number = 5
          ...
          maxWidth > 110
            ? (effect.platform.width = 110)
            : (effect.platform.width = maxWidth);
          break;
        }
        case 6:
          //power.number = 6
...
function UIElements({
  ...
  gameOverUI: React.RefObject<HTMLDivElement>;
  gameWonUI: React.RefObject<HTMLDivElement>;
  startGame: React.RefObject<HTMLDivElement>;
  ...
  return (
    <>
      <section className="ui">
        <div id="start" className="activate" ref={startGame}>
          <h1>Jardinains on dfinity</h1>
          <h2>Start game</h2>
          <button className="restart-btn" ref={startBtn}>
            Start
          </button>
        </div>
        <div id="gameover" ref={gameOverUI}>
          <h1>Game over!</h1>
          <div className="score">
            <h2>
              Coins:- <span className="score-count">0</span>
            </h2>
          </div>
          <button
        ...
        <div id="restart" ref={gameWonUI}>
          <h1>You won!</h1>
          <div className="score">
            <h2>
              Coins:- <span className="score-count">0</span>
            </h2>
            {showCoupon && <p>Coupon: {coupon}</p>}
          </div>
          <button
            className="restart-btn"
            onClick={() => window.location.reload()}
      ...
      <section className="hidden-assets">
        <div id="character">
          <img
            className="character"
